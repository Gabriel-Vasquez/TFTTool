...
- Phase: TFTTool 0.6.3 production-BETA correction is complete, package-validated, and published to the controlled stable update channel.
- Branch: `main`.
- Published release: TFTTool `0.6.3` on public `main`, with installer asset tag `v0.6.3` and `updates/stable.json` as the controlled stable channel.
...
- Version 0.6.3 guarantees that the verified exact six-region baseline remains the current dataset when a differently identified, newer partial snapshot such as 11,786 observations exists. The partial snapshot and all settings/history remain preserved. Team Planner exports use Riot client format `02` with fifteen hexadecimal slots, and the complete left sidebar is viewport-fixed so Settings remains visible while content scrolls.
...
1. Existing 0.6.2 installations apply 0.6.3 through the always-visible Settings > Update TFTTool control; new installations may use the v0.6.3 release installer.
...
- All 77 tests pass on 0.6.3, including the exact installed-startup reproduction with a newer 11,786-observation local snapshot, preservation of that snapshot and language setting, selection of the bundled 12,000-observation baseline, the accepted in-game Team Planner v2 example string, and the fixed-sidebar Settings contract.
...
- The self-contained 0.6.3 NSIS updater passed isolated packaged standalone QA: `dist/TFTTool Setup 0.6.3.exe` (89,468,107 bytes, SHA-256 `B1D4D84709BC27F556E886AB0A765CE5A08B4BDE1B3CD6F80A846AF5171868B4`). QA verified app version 0.6.3, exactly 12,000 observations, 25 cards, zero horizontal overflow, all 25 compact and all 37 expanded flagship/variant export codes in accepted `02`/15-slot syntax, Settings/update/import/export controls, and an unchanged Settings viewport position before and after scrolling 12,390 pixels. The sidebar computed as `fixed`; all hidden QA processes were shut down. The rebuilt bundled pack is 3,739,031 bytes with SHA-256 `80DDCA737156A7FA61D5F6BB251D54D9A079519B0FB2C745D33FD5381E8A6153`; its 12,000-observation source payload SHA-256 remains `dd3db003e06c1c4663d460a34504cbde5412d30191a72bf7b8140d65f5c14388`.
