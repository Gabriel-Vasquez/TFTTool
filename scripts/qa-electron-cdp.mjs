...
const emblemModel = (await command('Runtime.evaluate', { expression: `(async()=>{const analysis=(await (await fetch('/api/analysis')).json()).result;const variants=analysis.compositions.flatMap((composition)=>composition.variants);const replicator=analysis.compositions.find((composition)=>composition.id==='core:TFT17_MissFortune+TFT17_Ornn+TFT17_Viktor')?.variants.find((variant)=>['TFT17_MissFortune','TFT17_Lulu','TFT17_Nami'].every((id)=>variant.champions.some((champion)=>champion.id===id)));const namiComposition=analysis.compositions.find((composition)=>composition.id==='core:TFT17_Blitzcrank+TFT17_Nami+TFT17_Riven');const namiVariant2=namiComposition?.variants[1];return { archetypes:analysis.compositions.filter((composition)=>composition.requiresEmblem).length, variants:variants.filter((variant)=>variant.requiresEmblem).length, invalid:variants.filter((variant)=>variant.requiresEmblem&&!((variant.requiredEmblemCount===variant.sampleSize)||(variant.sampleSize>=30&&variant.requiredEmblemRate>=0.95))).length, namiVariant2:namiVariant2?{sampleSize:namiVariant2.sampleSize,requiresEmblem:namiVariant2.requiresEmblem,emblemRate:namiVariant2.emblemRate,requiredEmblemId:namiVariant2.requiredEmblemId,requiredEmblemCount:namiVariant2.requiredEmblemCount,requiredEmblemRate:namiVariant2.requiredEmblemRate}:null, replicatorCore:replicator?.coreChampions.map((champion)=>champion.id)||[], replicatorCoreItems:replicator?.coreChampions.reduce((total,champion)=>total+champion.itemSlots.slice(0,3).length,0)||0 };})()`, awaitPromise: true, returnByValue: true })).result.value;
if (emblemModel.variants < 1 || emblemModel.invalid !== 0 || emblemModel.namiVariant2?.sampleSize !== 130 || emblemModel.namiVariant2?.requiresEmblem !== true || emblemModel.namiVariant2?.requiredEmblemId !== 'TFT17_Item_SpaceGrooveEmblemItem' || emblemModel.namiVariant2?.requiredEmblemCount !== 127 || !emblemModel.replicatorCore.includes('TFT17_MissFortune') || !emblemModel.replicatorCore.some((id)=>['TFT17_Lulu','TFT17_Nami'].includes(id)) || emblemModel.replicatorCoreItems < 9) throw new Error(`Variant CORE or emblem evidence model failed: ${JSON.stringify(emblemModel)}`);
...
const namiVariant2Badge = (await command('Runtime.evaluate', { expression: `document.querySelector('[data-composition-id="core:TFT17_Blitzcrank+TFT17_Nami+TFT17_Riven"]')?.closest('.comp-card')?.querySelector('.variant-diff .emblem-badge')?.textContent || null`, returnByValue: true })).result.value;
if (namiVariant2Badge !== '[Emblema]') throw new Error(`Expected telemetry-tolerant emblem badge on Nami Variant 2, found ${JSON.stringify(namiVariant2Badge)}.`);
