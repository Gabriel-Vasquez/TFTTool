...
The bundled 0.6.16 baseline contains 24,000 ranked observations: 4,000 each from EUW, NA, KR, BR, LAN, and LAS. Challenger and Grandmaster evidence is collected first; Master is used only for any remaining regional deficit, and tier/LP provenance is retained for audit. Routine refreshes fetch only match IDs newer than each regional cursor, merge newly observed boards, and keep the newest 4,000 observations per region; a set/patch change automatically falls back to a complete deterministic collection. Windows in-place updates preserve the current installation directory and request the Windows elevation prompt when that directory is protected.
