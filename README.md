...
The bundled 0.6.14 baseline contains 24,000 ranked observations: 4,000 each from EUW, NA, KR, BR, LAN, and LAS. Challenger and Grandmaster evidence is collected first; Master is used only for any remaining regional deficit, and tier/LP provenance is retained for audit. Routine refreshes revalidate a bounded, concurrent elite window and retrieve only unseen match details; a set/patch change automatically falls back to a complete deterministic collection.
