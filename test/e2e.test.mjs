...
test('installed upgrade selects the canonical 24,000 baseline while preserving a newer 11,786-record local snapshot', async (t) => {
  ...
  assert.ok(snapshots.some((snapshot) => snapshot.dataset.id === 'set-18-live' && snapshot.observationCount === 24_000));
...
test('isolated settings flow persists language and layout and rejects malformed values and keys', async (t) => {
  ...
  const missingDataset = await fetch(`${url}/api/settings`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ datasetId: 'set-16-live' }) });
  assert.equal(missingDataset.status, 400);
  assert.equal((await missingDataset.json()).error, 'dataset_not_available');
...
test('isolated teammate flow exports and stages portable data without a Riot key or refresh', async (t) => {
  ...
  assert.equal(importResult.skippedSnapshots, 3);
  const parsed = parseDataPack(Buffer.from(pack));
  assert.deepEqual(parsed.snapshots.map((snapshot) => datasetIdentity(snapshot)).sort(), ['set-17-live', 'set-18-live', 'set-18-pbe']);
  ...
  assert.equal(updateResult.snapshots, 4);
