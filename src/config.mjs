...
// Temporary, owner-authorized make-up mode: when small regions cannot reach the
// per-region target, provider regions over-collect until the six-region total is met.
export const REGION_MAKEUP = process.env.TFTTOOL_REGION_MAKEUP === '1';
...
export const REGION_MAKEUP_PROVIDERS = Object.freeze(
  (process.env.TFTTOOL_REGION_MAKEUP_PROVIDERS ? process.env.TFTTOOL_REGION_MAKEUP_PROVIDERS.split(',').map((region) => region.trim().toUpperCase()).filter(Boolean) : ['EUW', 'NA', 'KR'])
