...
const COST_BY_RARITY = new Map([[0, 1], [1, 2], [2, 3], [4, 4], [6, 5]]);
export const costFromRarity = (value) => COST_BY_RARITY.get(Number(value)) || null;
export function unitCost(unit) {
  const explicit = Number(unit?.cost);
  if (Number.isInteger(explicit) && explicit >= 1 && explicit <= 5) return explicit;
  return costFromRarity(unit?.rarity);
}
...
export function normalizeParticipant(match, participant, region, ladder = {}) {
  ...
  const units = (participant.units || []).filter(displayableUnit).map((unit) => {
    const rarity = validNumber(unit.rarity);
    return {
